	...
	void RespondToDump(TextDump dump) {
		...
		for (int i = 0; i < dumpAffinities.Count && !dumpFound; i++) {
			if (dumpAffinities[i].dump == dump) {
				dumpFound = true;
				if (liking >= dumpAffinities[i].minToLike && liking <= dumpAffinities[i].minToLike) {
					liking += dumpAffinities[i].likeChange;
				} else if (liking <= dumpAffinities[i].maxToDislike && liking <= dumpAffinities[i].maxToDislike) {
